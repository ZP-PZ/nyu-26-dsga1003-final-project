"""Evaluate the trained static residual model on validation and test splits."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate the trained static residual model."
    )
    repo_root = Path(__file__).resolve().parent.parent
    parser.add_argument(
        "--checkpoint-path",
        default=str(repo_root / "artifact" / "checkpoints" / "static_residual" / "trained_model.pt"),
        help="Path to the saved static residual checkpoint.",
    )
    parser.add_argument(
        "--model-dir",
        default=None,
    ...
    parser.add_argument(
        "--output-path",
        default=str(repo_root / "result" / "metrics" / "static_residual.json"),
...
def main() -> None:
    ...
    model, checkpoint = load_static_model_for_eval(
    ...
    metrics["model_type"] = "static_residual"
